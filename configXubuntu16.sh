...
sudo apt-get install -y -qq leafpad vlc browser-plugin-vlc kodi kdenlive audacity mixxx obs-studio
clear
...
if [ "$scis" != "n" ]; then
  ...
  sudo apt-get install -y -qq pyqt4-dev-tools python-sphinx bluefish
fi
clear
